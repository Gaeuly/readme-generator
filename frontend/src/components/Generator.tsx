...
import { Send, AlertTriangle, CheckCircle, Copy, Download, Eye, Edit3 } from 'lucide-react';
...
// --- Sub-component for editor controls (Copy, Download) ---
const EditorControls: React.FC<{ content: string; setStatus: React.Dispatch<React.SetStateAction<{ type: 'loading' | 'error' | 'success' | 'idle'; message: string }>> }> = ({ content, setStatus }) => {
    ...
    return (
        <div className="flex items-center justify-between p-3 border-b border-gray-700/50">
            ...
            <div className="flex items-center gap-x-2">
                <button onClick={handleCopy} className="text-gray-400 hover:text-white p-2 rounded-md transition-colors" aria-label="Copy Markdown">
                    {copied ? <CheckCircle size={16} className="text-green-400" /> : <Copy size={16} />}
                </button>
                <button onClick={handleDownload} className="text-gray-400 hover:text-white p-2 rounded-md transition-colors" aria-label="Download Markdown">
                    <Download size={16} />
                </button>
            </div>
        </div>
